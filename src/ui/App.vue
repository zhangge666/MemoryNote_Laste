<template>
  <div class="w-screen h-screen flex flex-col select-none bg-surface-50 dark:bg-surface-900">
    <!-- Title Bar -->
    <div class="drag titlebar h-12 flex items-center justify-between px-4 border-b border-surface-200 dark:border-surface-700 bg-white/95 dark:bg-surface-800/95 backdrop-blur-sm">
      <div class="flex items-center gap-3 no-drag">
        <div class="w-6 h-6 bg-gradient-to-br from-primary-500 to-accent-600 rounded-lg flex items-center justify-center">
          <div class="w-3 h-3 bg-white rounded-sm"></div>
        </div>
        <button class="px-3 py-1.5 text-sm rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors" @click="toggleLeft">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
          </svg>
        </button>
        <button class="px-3 py-1.5 text-sm rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors" @click="openSearch">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex items-center gap-2 no-drag">
        <div class="w-7 h-7 bg-surface-200 dark:bg-surface-600 rounded-full"></div>
        <button class="px-3 py-1.5 text-sm rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors" @click="toggleRight">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
          </svg>
        </button>
        <button class="w-8 h-8 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-lg transition-colors flex items-center justify-center" @click="minimize">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/>
          </svg>
        </button>
        <button class="w-8 h-8 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-lg transition-colors flex items-center justify-center" @click="maximize">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex overflow-hidden">
      <!-- Side Nav -->
      <div class="w-16 border-r border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800">
      ...
      <div
        class="h-full border-r border-surface-200 dark:border-surface-700 overflow-hidden flex flex-col bg-white dark:bg-surface-800"
      ...
      <div
        v-show="leftOpen"
        class="no-drag w-2 cursor-col-resize hover:bg-surface-200/70 dark:hover:bg-surface-700/70 transition-colors"
        :class="{ 'bg-surface-300/70 dark:bg-surface-600/70': isResizingLeft }"
      ...
      <div
        v-show="rightOpen"
        class="no-drag w-2 cursor-col-resize hover:bg-surface-200/70 dark:hover:bg-surface-700/70 transition-colors"
        :class="{ 'bg-surface-300/70 dark:bg-surface-600/70': isResizingRight }"
      ...
      <div
        class="h-full border-l border-surface-200 dark:border-surface-700 overflow-hidden flex flex-col bg-white dark:bg-surface-800"
...
console.log('⚙️ App.vue: Loading settings store');
settings.load();

const themeStore = useThemeStore();
console.log('🎨 App.vue: Initializing theme store');
// 先初始化默认主题，再加载设置
themeStore.initTheme();
console.log('🎨 App.vue: Loading theme from settings');
themeStore.loadFromSettings();

console.log('✅ App.vue: Application initialization completed');
