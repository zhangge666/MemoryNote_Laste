  ...
  <div class="h-full flex flex-col p-6">
    ...
    <div class="mb-6">
      <div class="relative">
        <input 
          ...
          type="text" 
          placeholder="搜索知识库..."
          class="w-full px-4 py-3 pl-12 text-sm border border-surface-200 dark:border-surface-600 rounded-xl bg-white dark:bg-surface-700 focus:border-primary-500 transition-all"
        />
        <svg class="absolute left-4 top-1/2 transform -transurface-y-1/2 w-5 h-5 text-surface-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div v-if="searchResults && searchResults.length > 0" class="mb-6">
        <h3 class="text-sm font-semibold text-surface-800 dark:text-surface-200 mb-3 flex items-center">
        ...
        <div class="space-y-3">
          <div 
            ...
            :key="r.id" 
            class="p-4 bg-white dark:bg-surface-700 rounded-xl border border-surface-200 dark:border-surface-600 hover:shadow-md transition-all cursor-pointer"
            @click="openResult(r)"
          >
            <div class="flex items-center justify-between mb-2">
              <div class="font-medium text-surface-800 dark:text-surface-200 text-sm">
                {{ basename(r.file) }}
              </div>
            ...
            <div class="text-sm text-surface-600 dark:text-surface-400 line-clamp-3">
              {{ r.text }}
            </div>
          </div>
        </div>
      ...
      <div v-if="llmResponse" class="mb-6">
        ...
        <div class="p-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-xl border border-purple-200 dark:border-purple-800">
          <div class="text-sm text-surface-700 dark:text-surface-300 whitespace-pre-wrap">{{ llmResponse }}</div>
        </div>
      </div>
    ...
    <div class="border-t border-surface-200 dark:border-surface-700 pt-4 mt-4">
      <button 
        @click="rebuildIndex" 
        class="w-full bg-surface-100 dark:bg-surface-700 hover:bg-surface-200 dark:hover:bg-surface-600 text-surface-700 dark:text-surface-300 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center"
      >
        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
